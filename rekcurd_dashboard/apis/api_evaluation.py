...
from venus912_dashboard.models import (db, ApplicationModel, ServiceModel, EvaluationModel,
...
from venus912_dashboard.utils import HashUtil, venus912DashboardException, ApplicationUserRoleException
from venus912_dashboard.auth import auth, fetch_application_role
...
class ApiEvaluations(Resource):
    ...
    def post(self, project_id: int, application_id: str):
        ...
        if data_server_model.data_server_mode == DataServerModeEnum.LOCAL:
            ...
            response_body = venus912_dashboard_client.run_upload_evaluation_data(file, eval_data_path)
            if not response_body['status']:
                raise venus912DashboardException('Failed to upload')
        else:
...
class ApiEvaluate(Resource):
    ...
    def post(self, project_id: int, application_id: str):
        ...
        if evaluation_result_model is not None:
            raise venus912DashboardException("The evaluation result already exists")
    ...
    def put(self, project_id: int, application_id: str):
        ...
        if evaluation_result_model is None:
            raise venus912DashboardException("The evaluation result does not exist yet")
    ...
    def _evaluate(self, application_id: str, service_model: ServiceModel, evaluation_model: EvaluationModel):
        ...
        venus912_dashboard_client = venus912DashboardClient(
            ...
            service_level=service_model.service_level, venus912_grpc_version=service_model.version)
        return venus912_dashboard_client.run_evaluate_model(evaluation_model.data_path, eval_result_path), eval_result_path
...
class ApiEvaluationResultId(Resource):
    ...
    def get(self, project_id: int, application_id: str, eval_result_id: int):
        ...
        response_body = list(venus912_dashboard_client.run_evaluation_data(
